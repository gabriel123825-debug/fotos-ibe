...
import './styles.css';
import logoIbe from './assets/logo-ibe.png';
...
const Logo=()=> <div className="brand"><img className="brandLogo" src={logoIbe} alt="Igreja Batista Emanuel"/></div>;

function PublicSite(){
 const [albums,setAlbums]=useState([]),[year,setYear]=useState(null);
 useEffect(()=>{supabase.from('albums').select('*').eq('is_visible',true).order('start_date',{ascending:false}).then(({data})=>{const ordered=(data||[]).sort((a,b)=>{const ya=new Date(a.start_date+'T12:00:00').getFullYear(),yb=new Date(b.start_date+'T12:00:00').getFullYear();if(ya!==yb)return yb-ya;if(a.date_type!==b.date_type)return a.date_type==='annual'?-1:1;return new Date(b.start_date+'T12:00:00')-new Date(a.start_date+'T12:00:00')});setAlbums(ordered);if(ordered.length)setYear(new Date(ordered[0].start_date+'T12:00:00').getFullYear())})},[]);
 ...
 <div className="googleCall"><div><b>Encontrou sua foto?</b><p><strong>Compartilhe com seus familiares e nas redes sociais.</strong> Vamos espalhar o que Deus tem feito no meio do seu povo!<br/>E não se esqueça de marcar a <strong>@ibemanuelvg</strong>.</p></div></div>
 </main><footer><Logo/><span>Igreja Batista Emanuel · Várzea Grande — MT</span><div className="footerLinks"><a href="https://ibemanuelvg.com.br/" target="_blank" rel="noreferrer">Site oficial</a><a href="https://www.instagram.com/ibemanuelvg/" target="_blank" rel="noreferrer">Instagram</a><a className="adminLink" href="/admin">Administração</a></div></footer></>
}
